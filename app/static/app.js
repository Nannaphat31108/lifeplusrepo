...
async function exportSourceExcel(id){
  if(!window.currentPersonAccess?.person_key){
    alert("กรุณาเลือกคนที่ 1-4 และใส่รหัสก่อน");
    return;
  }
  return exportExcel(`/api/source-forms/record/${id}/excel`);
}
...
async function loadExactAssets(){
 if(!exactFormsCache) exactFormsCache=await fetch("/static/exact_forms.json").then(r=>r.json());
 if(!exactFieldsCache) exactFieldsCache=await fetch("/static/exact_fields.json").then(r=>r.json());
 if(!window.supplementCodeData) try{window.supplementCodeData=await fetch("/static/supplement_codes.json").then(r=>r.json())}catch{window.supplementCodeData=[]}
}
...
function exactFieldMap(code,form){
 const map={};for(const f of (exactFieldsCache[code]||[])){map[topLeftAddr(form,f.cell)]=f}return map;
}
function exactInput(field,addr,cellValue){
 ...
 if(field.type==="number_auto") return `<input ${common} class="excel-input auto-editable-cell" type="number" step="0.000000001" placeholder="อัตโนมัติ / แก้เองได้" oninput="markFormulaOverride(this);${field.group==='items'?'recalculateAdminQP()':''}">`;
 if(field.type==="number") return `<input ${common} type="number" step="0.000001" placeholder="${esc(placeholder)}" oninput="${field.group==='items'?'recalculateAdminQP()':'recalculateFormulaBoth()'}" onchange="${field.group==='items'?'recalculateAdminQP()':'recalculateFormulaBoth()'}">`;
 ...
 if(field.type==="supplement") return `<input ${common} list="exactSupplementNameList" placeholder="ค้นหาชื่อสาร" oninput="${field.group==='items'?'linkQPIngredientInput(this)':(field.group==='inactive_ingredients'?'autoLinkInactiveIngredient(this)':'autoLinkIngredient(this)')};${field.group==='items'?'recalculateAdminQP()':'recalculateFormulaBoth()'}">`;
...
async function openExactFormLegacy(code){
 ...
 <datalist id="exactSupplementNameList">${supplements.map(x=>`<option value="${esc(x.name)}">${esc(x.code)} — ${esc(x.vendor||"")}</option>`).join("")}</datalist>
...
function inactiveIngredientCellField(code,addr){
  ...
  if(code==="F-RD-002.1" && row>=34 && row<=36){
    const idx=row-34;
    if(col==="D") return {group:"inactive_ingredients",index:idx,sub:"name",type:"supplement"};
    if(col==="P") return {group:"inactive_ingredients",index:idx,sub:"quantity_mg",type:"number"};
    if(col==="V") return {group:"inactive_ingredients",index:idx,sub:"production_kg",type:"number_auto"};
    if(col==="Z") return {group:"inactive_ingredients",index:idx,sub:"percent",type:"number_auto"};
    if(col==="AA") return {group:"inactive_ingredients",index:idx,sub:"price_kg",type:"number"};
    if(col==="AI") return {group:"inactive_ingredients",index:idx,sub:"supplier",type:"supplier"};
    if(col==="AM") return {group:"inactive_ingredients",index:idx,sub:"material_code",type:"supplement_code"};
  }
  return null;
}
...
function autoLinkInactiveIngredient(inp){
  ...
  const item=inp.dataset.sub==="material_code" ? findSupplementByCode(inp.value) : findSupplementByName(inp.value);
  if(!item)return;
  const set=(sub,val)=>{
    const e=document.querySelector(`.excel-input[data-group="inactive_ingredients"][data-index="${i}"][data-sub="${sub}"]`);
    if(e)e.value=val??"";
  };
  set("material_code",item.code||"");
  set("name",item.name||"");
  set("supplier",item.vendor||"");
  set("import_country",item.origin||"");
  set("price_kg",item.price||"");
  set("halal",item.halal||"");
  recalculateFormulaBoth();
}

function editableTableBlankCell(code,addr,ce){
  if(code!=="F-RD-002" && code!=="F-RD-002.1" && code!=="ADMIN-QP")return null;
  const v=ce?.v;
  const hasTableStyle=!!ce?.fill || !!(ce?.border && Object.keys(ce.border).length);
  if(!hasTableStyle)return null;
  if(v!=="" && v!==null && v!==undefined && v!=="#DIV/0!" && String(v)!=="0")return null;
  const isNum=(v==="#DIV/0!" || String(v)==="0");
  return `<input class="excel-input manual-cell-input generic-excel-edit" data-manual-cell="${addr}" type="${isNum?"number":"text"}" ${isNum?'step="0.000001"':""} value="${(v==="#DIV/0!"||String(v)==="0")?"":esc(v||"")}" placeholder="พิมพ์ข้อมูล" oninput="recalculateFormulaBoth()">`;
}

const MANUAL_EDIT_CELLS={
 "F-RD-002":{
   // Inactive Ingredient table
   "D39":"text","T39":"number","Z39":"number","AD39":"number","AE39":"number","AI39":"number","AM39":"text","AR39":"text","AS39":"text","AT39":"text",
   "D40":"text","T40":"number","Z40":"number","AD40":"number","AE40":"number","AI40":"number","AM40":"text","AR40":"text","AS40":"text","AT40":"text",
   "D41":"text","T41":"number","Z41":"number","AD41":"number","AE41":"number","AI41":"number","AM41":"text","AR41":"text","AS41":"text","AT41":"text",
...
function formulaAutoInputForCell(code,addr,currentValue){
  ...
  const val=(currentValue===undefined||currentValue===null||String(currentValue)==="#DIV/0!")?"":String(currentValue);
  return `<input class="excel-input manual-cell-input formula-auto-input auto-editable-cell" data-manual-cell="${addr}" data-calc-cell="${addr}" type="number" step="0.000001" value="${esc(val==="0"?"":val)}" placeholder="อัตโนมัติ / แก้เองได้" oninput="markFormulaOverride(this)">`;
}
...
function recalculateFormulaBoth(){
...
function qpExactEl(i,sub){return document.querySelector(`.excel-input[data-group="items"][data-index="${i}"][data-sub="${sub}"]`)}
function recalculateAdminQP(){
 ...
 document.querySelectorAll('.excel-input[data-group="items"]').forEach(()=>{});

 for(let i=0;i<15;i++){
   const row=document.querySelector(`.excel-input[data-group="items"][data-index="${i}"][data-sub="quantity"]`)?.closest("tr");
   const qmg=document.querySelector(`.excel-input[data-group="items"][data-index="${i}"][data-sub="quantity_mg"]`);
   const qty=document.querySelector(`.excel-input[data-group="items"][data-index="${i}"][data-sub="quantity"]`);
   const price=document.querySelector(`.excel-input[data-group="items"][data-index="${i}"][data-sub="unit_price"]`);
   const amount=document.querySelector(`.excel-input[data-group="items"][data-index="${i}"][data-sub="amount"]`);

   const qmgVal=Number(qmg?.value||0)||0;
   const qtyVal=Number(qty?.value||0)||0;
   const priceVal=Number(price?.value||0)||0;

   totalQty+=qtyVal || qmgVal;

   const rowAmount=qtyVal*priceVal;
   if(amount && amount.dataset.manualOverride!=="1"){
     amount.value=fmtCalc(rowAmount,2);
   }
   subtotal+=Number(amount?.value||rowAmount||0)||0;
 }

 const get=k=>document.querySelector(`.excel-input[data-key="${k}"]`);
 const set=(k,v,d=2)=>{
   const e=get(k);
   if(e && e.dataset.manualOverride!=="1")e.value=fmtCalc(v,d);
 };

 // No fixed discount rate exists in the source file; keep after-discount equal
 // to subtotal until user overrides it manually.
 const afterDiscountEl=get("after_discount");
 let afterDiscount=subtotal;
 if(afterDiscountEl?.dataset.manualOverride==="1"){
   afterDiscount=Number(afterDiscountEl.value||0)||0;
 }

 const vat=afterDiscount*0.07;
 const grand=afterDiscount+vat;

 set("total_quantity",totalQty,3);
 set("subtotal",subtotal,2);
 set("after_discount",afterDiscount,2);
 set("vat7",vat,2);
 set("grand_total",grand,2);
 set("installment_1",grand*0.50,2);
 set("installment_2",grand*0.50,2);
}
function linkQPIngredientInput(inp){
 ...
 const name=qpExactEl(i,"ingredient_name"),unit=qpExactEl(i,"unit"),price=qpExactEl(i,"unit_price");
 if(name)name.value=item.name||inp.value;if(unit&&!unit.value)unit.value=item.unit||"";if(price&&!price.value)price.value=item.price||"";recalculateAdminQP()
}
async function linkAdminQPFormula(force=false){
 const formulaNo=String(document.getElementById("qpExactFormulaNo")?.value||"").trim();if(!formulaNo)return;
 try{const linked=await api(`/api/source-forms/formula-link/${encodeURIComponent(formulaNo)}`);adminQPFormulaNo=formulaNo;
 const setKey=(k,v)=>{const e=document.querySelector(`.excel-input[data-key="${k}"]`);if(e&&!String(e.value||"").trim())e.value=v??""};setKey("customer_name",linked.customer_name||"");setKey("product_name",linked.product_name||"");
 const all=[...(linked.ingredients||[]),...(linked.inactive_ingredients||[])];
 for(let i=0;i<15;i++){const x=all[i],desc=qpExactEl(i,"description"),name=qpExactEl(i,"ingredient_name"),qtymg=qpExactEl(i,"quantity_mg"),unit=qpExactEl(i,"unit"),price=qpExactEl(i,"unit_price");if(!x){if(force)for(const el of [desc,name,qtymg,unit,price])if(el)el.value="";continue}const master=findSupplementByCode(x.material_code)||findSupplementByName(x.name)||{};if(desc)desc.value=x.type==="ACTIVE"?"สารสกัดสำคัญ":"ส่วนประกอบไม่สำคัญ";if(name)name.value=x.name||master.name||"";if(qtymg)qtymg.value=x.quantity_mg??"";if(unit)unit.value=x.unit||master.unit||"";if(price)price.value=x.price_kg||master.price||""}
 recalculateAdminQP();toast(`ลิงก์เลขที่สูตร ${linked.formula_no} แล้ว • ${all.length} รายการ`)}catch(err){if(force)alert("ลิงก์เลขที่สูตรไม่ได้: "+(err?.message||err))}
}
function collectAdminQPExactFormulaNo(d){if(currentExactForm==="ADMIN-QP")d.formula_no=adminQPFormulaNo||document.getElementById("qpExactFormulaNo")?.value||"";return d}
...
async function openPrivateExactForm(code){
  ...
  $("pageContent").innerHTML = `
    <div class="exact-form-toolbar">
      ...
      <div class="actions">
        ...
        ${(code==="ADMIN-QP")?`<div class="qp-exact-link"><input id="qpExactFormulaNo" placeholder="เลขที่สูตร F-RD-002"><button onclick="linkAdminQPFormula(true)">ดึงข้อมูลสูตร</button></div>`:""}
  ...
  if(code==="F-RD-002" || code==="F-RD-002.1") setTimeout(recalculateFormulaBoth,0);
  if(code==="ADMIN-QP") setTimeout(recalculateAdminQP,0);
}

function buildAliasPanel(code){ return ""; }
function findSupplementByCode(code){
 return (window.supplementCodeData||[]).find(x=>String(x.code||"").trim().toLowerCase()===String(code||"").trim().toLowerCase());
}
function findSupplementByName(name){
 return (window.supplementCodeData||[]).find(x=>String(x.name||"").trim().toLowerCase()===String(name||"").trim().toLowerCase());
}
function applyLinkedMaterial(index,item){
 if(!item)return;
 const setGroup=(sub,val)=>{
   const e=document.querySelector(`.excel-input[data-group="ingredients"][data-index="${index}"][data-sub="${sub}"]`);
   if(e)e.value=val??"";
 };
 setGroup("material_code",item.code||"");
 setGroup("name",item.name||"");
 setGroup("supplier",item.vendor||"");
 setGroup("import_country",item.origin||"");
 setGroup("price_kg",item.price||"");
 setGroup("halal",item.halal||"");
 recalculateFormulaBoth();
}
...
function linkedPanelNameChanged(inp){
 const i=Number(inp.dataset.index),item=findSupplementByName(inp.value);
 if(item)applyLinkedMaterial(i,item);
}
...
function populateExactForm(d){
 ...
 (d.ingredients||[]).forEach((x,i)=>{
   for(const [k,v] of Object.entries(x||{})){
     const e=document.querySelector(`.excel-input[data-group="ingredients"][data-index="${i}"][data-sub="${k}"]`);
     if(e && v!==undefined && v!==null)e.value=v;
   }
   const map={
     ".alias-code":x.material_code,
     ".alias-main":x.name,
 ...
 setTimeout(recalculateFormula,0);
}
...
saveExactForm=async function(code){
 try{
   ...
   const no=$("exactRecordNo").value.trim()||`${code}-${Date.now()}`;
   const body={record_no:no,status:"DRAFT",data:collectExactPayload()};
   if(window.editingSourceRecordId){
     const r=await api(`/api/source-forms/record/${window.editingSourceRecordId}`,{method:"PUT",body});
     toast(`บันทึกการแก้ไข ${r.record_no||no} แล้ว`);
   }else{
     const r=await api(`/api/source-forms/${code}`,{method:"POST",body});
     window.editingSourceRecordId=r.id;
     toast(`บันทึก ${r.record_no||no} แล้ว`);
   }
   await showSourceRecords(code);
 }catch(e){
   console.error("Save exact form failed:",e);
...
async function openDepartmentWorkspace(code){
 ...
 ADMIN:{title:"ADMIN",text:"บริหารผู้ใช้ เอกสาร และข้อมูลกลาง",cards:[["QP / Quotation","ฟอร์ม QP ต้นฉบับ • ลิงก์สูตร / คำนวณอัตโนมัติ","openExactForm(\\'ADMIN-QP\\')"],["Users / Audit","จัดการผู้ใช้และประวัติระบบ","openPage('admin')"],["Original Forms","เอกสารต้นฉบับ","openPage('originalForms')"],["Customers","ฐานข้อมูลลูกค้า","openPage('customers')"]]},
 ...
 PURCHASE:{title:"PURCHASE",text:"Supplier และการจัดซื้อ",cards:[["Suppliers","ฐาน Supplier","openPage('suppliers')"],["Stock Requirement","ตรวจความต้องการวัตถุดิบ","openPage('inventory')"]]},
...
document.addEventListener("input",function(e){
  ...
  const isFormula=currentExactForm==="F-RD-002" || currentExactForm==="F-RD-002.1";
  if(!isFormula)return;

  if(
    el.dataset.sub==="quantity_mg" ||
    el.dataset.sub==="price_kg" ||
    el.dataset.key==="order_quantity"
  ){
    requestAnimationFrame(()=>{
      if(el.dataset.sub==="quantity_mg" || el.dataset.sub==="price_kg"){
        forceUpdateRowCost(Number(el.dataset.index), el.dataset.group||"ingredients");
      }
      recalculateFormulaBoth();
    });
  }
...
// LIVE RECALC FALLBACK for browser number spinner/change/autofill.
document.addEventListener("change",function(e){
  const el=e.target;
  if(!el?.classList?.contains("excel-input"))return;
  if(currentExactForm!=="F-RD-002" && currentExactForm!=="F-RD-002.1")return;
  if(el.dataset.sub==="quantity_mg" || el.dataset.sub==="price_kg" || el.dataset.key==="order_quantity"){
    recalculateFormulaBoth();
  }
},true);

function forceUpdateRowCost(i, group="ingredients"){
  const get=sub=>document.querySelector(`.excel-input[data-group="${group}"][data-index="${i}"][data-sub="${sub}"]`);
  const qty=Number(get("quantity_mg")?.value||0)||0;
  const price=Number(get("price_kg")?.value||0)||0;
  const result=price/1000000*qty;
  const out=get("row_cost");
  if(out){
    out.dataset.manualOverride="";
    out.value=fmtCalc(result,9);
  }
}
